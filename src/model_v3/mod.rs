...
impl TaggedModelRequest {
    #[instrument(level = "trace")]
    pub(super) fn new(tag: Uuid, user: Option<Uuid>, mut request: Value) -> TaggedModelRequest {
        if let Some(request) = request.as_object_mut() {
            match user {
                Some(user) => {
                    request.insert("user".to_string(), Value::String(format!("{}", user)))
                }
        ...
        TaggedModelRequest { tag, request }
    }
...
#[derive(Debug)]
pub(super) struct TaggedModelResponse {
    pub(super) tag: Uuid,
...
impl TaggedModelResponse {
    #[instrument(level = "trace")]
    pub(super) fn from_error(tag: Uuid, error: &ModelError) -> TaggedModelResponse {
        let response = match error {
            ...
            ModelError::OversizedRequest => json!({
                "message": "Your messages exceeded the model's maximum context length. Please reduce the length of the message. If you belive you are seeing this message in error, contact the proxy's administrator.",
                "type": "invalid_request_error",
                "param": Value::Null,
                "code": Value::Null,
            }),
        ...
        let status = match error {
            ...
            ModelError::OversizedRequest => StatusCode::PAYLOAD_TOO_LARGE,
...
pub(super) enum ModelBackendConfiguration {
...
#[derive(Debug)]
pub(super) struct ModelBackend {
    config: ModelBackendConfiguration,
    client: ModelBackendClient,
}

// TODO: Setup ClientBuilder

impl ModelBackend {
    #[instrument(level = "debug")]
    pub(super) fn init(config: ModelBackendConfiguration, http_client: &Client) -> ModelBackend {
        match &config {
            ModelBackendConfiguration::OpenAI(_) => ModelBackend {
                config,
                client: ModelBackendClient::Http(http_client.clone()),
            },
            ModelBackendConfiguration::Loopback => ModelBackend {
                config,
                client: ModelBackendClient::Stateless,
            },
        }
    }
    ...
    pub(super) fn get_max_tokens(&self) -> Option<u64> {
        match &self.config {
            ModelBackendConfiguration::OpenAI(backend) => backend.model_context_len,
            ModelBackendConfiguration::Loopback => None,
        }
    }
    ...
    pub(super) async fn generate(
        ...
        let request_tag = Value::String(format!("{}", tagged_request.tag));

        match &self.config {
            // TODO: Support Audio models (form-based requests, binary output)
            ModelBackendConfiguration::OpenAI(config) => {
                if let Some(request) = tagged_request.request.as_object_mut() {
                    ...
                    let client = if let ModelBackendClient::Http(client) = &self.client {
                        client.clone()
                    } else {
                        tracing::warn!(
                            "Unable to access connection pool! This will lead to poor performance!"
                        );
                        Client::new()
                    };
                    ...
                    match url {
                        Ok(url) => {
                            let mut builder = client.post(url).bearer_auth(&config.openai_api_key);
                            ...
                            match builder.send().await {
                                Ok(response) => {
                                    ...
                                    if status.is_server_error() {
                                        ...
                                        return TaggedModelResponse::from_error(
                                            tagged_request.tag,
                                    ...
                                    match body {
                                        Ok(body) => match serde_json::from_str::<Value>(&body) {
                                            ...
                                            Err(error) => {
                                                ...
                                                TaggedModelResponse::from_error(
                                                    tagged_request.tag,
                                                    &ModelError::BackendError,
                                                )
                                            }
                    ...
                    TaggedModelResponse::from_error(tagged_request.tag, &ModelError::BadRequest)
                }
            }
            ModelBackendConfiguration::Loopback => TaggedModelResponse {
                tag: tagged_request.tag,
                usage: None,
